...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.jb74kx3.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    // get user/read many data
    app.get("/user", async (req, res) => {
      const coursor = userCollection.find();
      const restlt = await coursor.toArray();
    ...
    app.delete("/user/:id", async (req, res) => {
      ...
      console.log("please delet this : ", id);
...
app.get("/", (req, res) => {
  res.send("Hello World! this is tteeest express ");
});
